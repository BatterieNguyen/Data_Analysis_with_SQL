...
* __Convert timestamp in UTC to PST__
```
SELECT '2020-09-01 00:00:00' at time zone 'pst'
FROM	current_timezone;
```
* __changing the format of dates and times__ => `CONVERT()`
Example
```
...
```
Manipulate table to convert into timestamp
```
UPDATE YourTable
SET timestamp_column = CONVERT(DATETIME, '2022-03-17 09:30:45', 120)
WHERE condition;
```
```
INSERT INTO YourTable (timestamp_column)
VALUES (CONVERT(DATETIME, '2022-03-17 09:30:45', 120));
```
Use command function to analyze the data
```
SELECT CONVERT(DATETIME, your_text_column, 120) AS converted_timestamp
FROM YourTable;
```
```
...
---------- | ------------ | ---------------
`DATEFROMPARTS()` | ```SELECT DATEFROMPARTS(2018, 10, 31) AS DateFromParts; ``` | Return to date from parts

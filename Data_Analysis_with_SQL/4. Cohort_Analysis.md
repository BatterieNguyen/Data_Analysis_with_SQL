...
```
-- Step 1
SELECT	DATEPART(YEAR, a.first_term) AS fisrt_year,
		COALESCE(DATEDIFF(YEAR, a.first_term, b.term_start), 0) AS period,
		COUNT(DISTINCT a.id_bioguide) AS cohort_retained
FROM	(
		SELECT  id_bioguide, MIN(term_start) AS first_term
		FROM	legislator_terms
...
GROUP BY	DATEPART(YEAR, a.first_term),
			COALESCE(DATEDIFF(YEAR, a.first_term, b.term_start), 0)
ORDER BY	1, 2 ASC; 
```
-- Step 2   

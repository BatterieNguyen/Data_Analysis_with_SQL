...
10 years or five terms is arbitrary => It is possible to calculate the survivorship for each number of years or periods.
```
SELECT	a.century, b.term, 
		...
		COUNT(DISTINCT a.id_bioguide)*1.0
			/COUNT(DISTINCT CASE WHEN a.total_terms >= b.term THEN id_bioguide END) AS pct_survived
FROM
	(
	SELECT	id_bioguide,
			MIN(term_start) AS first_term,
...
- Survivorship considers only whether an entity was present as of a specific period or later.
----------------------------------------------------------------------------------------------------
